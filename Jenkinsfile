node{
  ...
  stage("Remove"){
     ...
     def dockerRm1 = 'docker image rm -f kayeofhallownest/webtest:latest'   
     sshagent(['ssh']) {
  ...
  stage("Build docker image"){
    sh 'docker build -t kayeofhallownest/webtest:latest .'
  }
  
  stage("Push docker image"){
    sh "docker login -u kayeofhallownest -p pankaye1999"
    sh 'docker push kayeofhallownest/webtest:latest'
  }
  stage("Deploy docker image to Tomcat server"){
    def dockerRun = 'docker run -p 8080:8080 -d --name web-test kayeofhallownest/webtest:latest'
    sshagent(['ssh']) {
